...
void run_reco_clusterizer(Int_t nEvents = 1)
{
  ...
  */
  ...
  // -----   TRD track fitting   ---------------------------------------------
  /* CbmTrdTrackFitter class has been removed
  ...
  */
  ...
  CbmFindGlobalTracks* findGlobal = new CbmFindGlobalTracks(trackMerger, NULL, tofMerger, iVerbose);
  run->AddTask(findGlobal);

  //--------- TOF tracklength calculation -----------------------------------
  CbmGlobalTrackFitterKF* globalFitter = new CbmGlobalTrackFitterKF();
  CbmFitGlobalTracks* fitGlobal        = new CbmFitGlobalTracks("globalfitter", iVerbose, globalFitter);
