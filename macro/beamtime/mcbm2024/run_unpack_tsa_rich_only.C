                    ...
                    std::string outpath = "data/", bool bIgnoreOverlapMs = true)
{
  ...
  if (outpath.empty()) {
    outpath = infile[0].substr(0, filenamepos);
  }
...
std::shared_ptr<CbmTrdUnpackMonitor> GetTrdMonitor(std::string treefilename, bool fasp = false)
{
  ...
  if (filetypepos != outfilename.npos)
    outfilename.replace(filetypepos, 10, ".mon.trd.root");
  else
...
std::shared_ptr<CbmStsUnpackMonitor> GetStsMonitor(std::string treefilename, bool bDebugMode = false)
{
  ...
  auto filetypepos        = outfilename.find(".digi.root");
  if (filetypepos != outfilename.npos)
    outfilename.replace(filetypepos, 10, ".mon.sts.root");
  else
...
std::shared_ptr<CbmMuchUnpackMonitor> GetMuchMonitor(std::string treefilename, bool bDebugMode = false)
{
  ...
  auto filetypepos        = outfilename.find(".digi.root");
  if (filetypepos != outfilename.npos)
    outfilename.replace(filetypepos, 10, ".mon.much.root");
  else
...
std::shared_ptr<CbmRichUnpackMonitor> GetRichMonitor(std::string treefilename, bool bDebugMode = false)
{
  ...
  auto filetypepos        = outfilename.find(".digi.root");
  if (filetypepos != outfilename.npos)
    outfilename.replace(filetypepos, 10, ".mon.rich.root");
  else
...
void run_unpack_tsa_rich_only(std::string infile = "test.tsa", UInt_t runid = 0,
                              std::string setupName = defaultSetupName, std::int32_t nevents = -1,
                              bool bBmoninTof = false, std::string outpath = "data/", bool bIgnoreOverlapMs = false)
{
