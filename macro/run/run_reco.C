 ...
 **
 **/
void run_reco(TString input = "urqmd_mbias", Int_t nTimeSlices = 1000, Int_t firstTimeSlice = 0, TString output = "",
              ...
              Bool_t debugWithMC = true)
{
  ...
  if (useMvd || useSts) {
    ...
    {
      TString cfgFile             = srcDir + "/macro/L1/configs/material_budget_example.yaml";
      auto* pMaterialMapGenerator = new CbmGenerateMaterialMaps();
      pMaterialMapGenerator->SetUserConfig(cfgFile);
      pMaterialMapGenerator->SetOutputName("material.root");
      //run->AddTask(pMaterialMapGenerator);
    }
