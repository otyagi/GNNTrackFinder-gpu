 ...
 **/
void run_tra_urqmd(const char* input = "urqmd.pau.28.3gev.mbias.00001.root", Int_t nEvents = 1000, const char* output = "urqmd_pau28.3gev_mbias_00001", const char* setup = "sis100_electron", ECbmEngine engine = kGeant3, int randomSeed = 0, Bool_t overwrite = kTRUE)
{
  ...
  if (dataset.IsNull()) dataset = "urqmd";
  ...
  // -----   Register HN   -------------------------
  std::cout << std::endl;
  TString registerHNMacro = gSystem->Getenv("VMCWORKDIR");
  registerHNMacro += "/macro/KF/registerHypernuclei.C";
  std::cout << "Loading macro " << registerHNMacro << std::endl;
  gROOT->LoadMacro(registerHNMacro);
  gROOT->ProcessLine("registerHypernuclei()");
  // ------------------------------------------------------------------------
