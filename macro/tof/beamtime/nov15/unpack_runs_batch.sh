...
if [ -z "$1" ]; then
  if [ ! -z "$SLURM_SUBMIT_DIR" ]; then
    ...
    echo 'can unpack multiple runs on the batch farm only'
    exit 1
  fi
else
  RunId=$1
...
root -b -q ''${VMCWORKDIR}'/macro/tof/beamtime/nov15/create_calib.C('${nEvents}',"CbmTofSps_'${RunId}'")'
root -b -q ''${VMCWORKDIR}'/macro/tof/beamtime/nov15/apply_calib.C('${nEvents}',"CbmTofSps_'${RunId}'")'

rm -v all_*
rm -v unpack_CbmTofSps_${RunId}.buffer.root

if [ ! -z "$SLURM_SUBMIT_DIR" ]; then
  mv -v ${wdir}/slurm-${SLURM_ARRAY_JOB_ID}_${SLURM_ARRAY_TASK_ID}.out ${outdir}/unpack_CbmTofSps_${RunId}.log
else
  cd ${origdir}
fi
