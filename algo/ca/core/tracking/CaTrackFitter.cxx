...
namespace cbm::algo::ca
{
  ...
  void TrackFitter::FitGNNTracklets(const ca::InputData& input, WindowData& wData, Vector<Track>& trackCandidates,
                                    Vector<HitIndex_t>& trackHits, Vector<int>& selectedTrackIndexes,
                                    Vector<float>& selectedTrackScores,
                                    std::vector<std::vector<float>>& selectedTrackParams, const int GNNiteration)
  {
    // LOG(info) << "Start GNN Tracklet Fitter";
    ...
    unsigned int N_vTracks = trackCandidates.size();  // number of tracks processed per one SSE register

    for (unsigned int itrack = 0; itrack < N_vTracks; itrack += fvec::size()) {

      if (N_vTracks - itrack < static_cast<unsigned int>(fvec::size())) nTracks_SIMD = N_vTracks - itrack;

      for (int i = 0; i < nTracks_SIMD; i++) {
        t[i] = &trackCandidates[itrack + i];
      }

      // fill the rest of the SIMD vectors with something reasonable
      for (uint i = nTracks_SIMD; i < fvec::size(); i++) {
        t[i] = &trackCandidates[itrack];
      }
      ...
      for (int iVec = 0; iVec < nTracks_SIMD; iVec++) {
        ...
        for (int ih = 0; ih < nHitsTrack; ih++) {
          const ca::Hit& hit           = input.GetHit(trackHits[start_hit++]);
          const int ista               = hit.Station();
          auto [detSystemId, iStLocal] = fParameters.GetActiveSetup().GetIndexMap().GlobalToLocal<EDetectorID>(ista);
      ...
      /// for debug
      /// print chi2 of the tracks from fit
      {
        // fvec chi2 = fit.Tr().GetChiSq();
        // for (int iVec = 0; iVec < nTracks_SIMD; iVec++) {
        // std::cout << "chi2 of track " << itrack + iVec << " is " << chi2[iVec] << '\n';
        // }
      }

      /// if track chi2 per dof is larger than threshold. Also kill negative and non-finite values
      /// if track p low than threshold_qp, then kill the track
      /// then remove the track from the list
      {
        // Change these based on iteration
        // for Secondary iteration
        // const std::vector<float> chi2_cuts{100, 40, 40, 50, 50, 50, 50, 50, 50};  // ndf - 3 to 19
        const std::vector<float> chi2_cuts{100, 100, 100, 50, 50, 50, 50, 50, 50};  // ndf - 3 to 19

        float threshold_chi2 = 100;
        float threshold_qp   = 10.0f;  // def - 10

        if (GNNiteration == 1) {   // All primary
          threshold_chi2 = 50.0f;  // def - 10.0f
          threshold_qp   = 10.0f;  // def - 10.0f
        }
        ...
        for (int iVec = 0; iVec < nTracks_SIMD; iVec++) {
          ...
          if (GNNiteration == 3) {
            const int ndf       = fit.Tr().Ndf()[iVec];
            const int cut_index = (ndf - 1) / 2 - 1;
            // threshold_chi2      = chi2_cuts[cut_index];
          }

          // chi2 cut
          bool killTrack = (chi2[iVec] > threshold_chi2) || !std::isfinite(chi2[iVec]) || chi2[iVec] < 0;
          ...
          /// C00 and C11 cut
          if (fit.Tr().C00()[iVec] > 1e-5 || fit.Tr().C11()[iVec] > 1e-3) {
            killTrack = true;
          }

          // // C22 and C33 cut
          if (fit.Tr().C22()[iVec] > 0.01 || fit.Tr().C33()[iVec] > 0.01) {
            killTrack = true;
          }
          ...
          if (!killTrack) {
            // std::cout << "chi2 of track length " << trackCandidates[itrack + iVec].fNofHits << " is " << chi2[iVec]
            //           << '\n';
            selectedTrackIndexes.push_back(itrack + iVec);
            selectedTrackScores.push_back(chi2[iVec]);

            const float Tx  = fit.Tr().Tx()[iVec];
            const float Ty  = fit.Tr().Ty()[iVec];
            const float p   = 1. / fit.Tr().Qp()[iVec];
            const float C00 = fit.Tr().C00()[iVec];
            const float C11 = fit.Tr().C11()[iVec];
            const float C22 = fit.Tr().C22()[iVec];
            const float C33 = fit.Tr().C33()[iVec];
            const float C44 = fit.Tr().C44()[iVec] + 0.001;  // 0.001 magic number added. (see triplet constructor)
            const float ndf = fit.Tr().Ndf()[iVec];
            const float x   = fit.Tr().X()[iVec];
            const float y   = fit.Tr().Y()[iVec];
            const float z   = fit.Tr().Z()[iVec];
            std::vector<float> trackParams{chi2[iVec], Tx, Ty, p, C00, C11, C22, C33, C44, ndf, x, y, z};
            selectedTrackParams.push_back(trackParams);
          }
        }
      }
